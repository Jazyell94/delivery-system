...
const allowedOrigins = ['https://jazyell94.github.io/delivery-system/', 'http://localhost:5173'];
      ...
      Promise.all(queries)
        .then(() => {
          db.commit((err) => {
            ...
            broadcastToClients({ action: 'newOrder', data: req.body });
...
app.put('/status/:clientId', (req, res) => {
  ...
  db.query(query, [status, clientId], (error) => {
    ...
    broadcastToClients({ action: 'updateStatus', clientId, status });
